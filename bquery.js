...
(function() {
  ...
  bQuery.view = function() {
    return new bQueryView;
  };

  bQuery.view.mixin = function(name, mixin) {
    var go, key, val, _results;
    go = function(n, m) {
      if (!bQuery.allowOverwrite) {
        if (bQueryView.prototype[n]) {
          throw "bQuery plugin with the name '" + n + "' already exists. Set bQuery.allowOverwrite = true to allow overwriting plugins";
        }
      }
      return bQueryView.prototype[n] = function() {
        var xs;
        xs = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
        return this.use(m.apply(null, xs));
      };
    };
    if (typeof name === 'object') {
      ...
      for (key in name) {
        ...
        val = name[key];
        _results.push(go(key, val));
      }
      return _results;
    } else {
      return go(name, mixin);
    }
  };
